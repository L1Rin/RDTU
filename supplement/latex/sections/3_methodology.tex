...
\subsection{Overview}
We propose \textbf{Reinforced Direct Temporal Unification (RDTU)}, a framework that exploits the intrinsic reasoning ability of Large Language Models (LLMs) for time series forecasting without complex modality alignment modules. As shown in \shortautoref{fig:method_framework}, RDTU follows a progressive two-stage training pipeline. First, \textbf{Supervised Temporal Instruction Tuning (STIT)} builds structure-aware instruction data to endow the model with basic forecasting ability and domain-specific instruction following, as detailed in \shortautoref{sec:stit}. Second, \textbf{Reinforcement-Driven Forecasting Refinement (RDFR)} further improves regression precision through task-specific rewards that enforce output format and refine prediction accuracy, as described in \shortautoref{sec:rdfr}.

\subsection{Supervised Temporal Instruction Tuning}\label{sec:stit}

To establish foundational capabilities for time-series reasoning and strict instruction adherence, we initiate the pipeline with \textbf{Supervised Temporal Instruction Tuning (STIT)}. Diverging from methods that rely on auxiliary encoders, we employ a direct serialization strategy where historical numerical data is transformed into discrete text strings. These sequences are seamlessly integrated into a structured natural language prompt composed of four semantic elements: an expert persona definition, domain-specific metadata, the serialized historical sequence, and the specific prediction directive. This unified textual format allows the pre-trained LLM backbone to be fine-tuned directly via standard causal language modeling, as detailed in \shortautoref{appx:prompt}.

\input{figures/Method}

\subsection{Reinforcement-Driven Forecasting Refinement}\label{sec:rdfr}
While STIT provides a good initialization, the model often struggles with strict numerical output regarding the formats and long-horizon precision. To address these residual issues, we introduce the \textbf{RDFR} system, which is composed of two core contributions: a \textit{Priority-Based Dual-Stream Hard Sample Mining} strategy for constructing high-leverage RDFR training data, and a \textit{Temporal Refinement Reward System} designed to guide the optimization process.

\subsubsection{RDFR Data Construction Strategy}

To address the instruction-following failures and precision deficits that persist after the STIT stage, we propose a \textbf{Priority-Based Dual-Stream Hard Sample Mining} strategy. As illustrated in \shortautoref{fig:RDFR}, we decouple the data selection into two parallel streams, Syntactic Correction ($\mathcal{D}_{syn}$) and Semantic Reinforcement ($\mathcal{D}_{sem}$), which are unified to form the final Reinforcement-Driven Forecasting Refinement Dataset $\mathcal{D}_{RDFR} = \mathcal{D}_{syn} \cup \mathcal{D}_{sem}$.
The complete algorithm of data construction strategy is shown in \shortautoref{appx:sample-mining}.


\paragraph{Syntactic Correction Stream.}
...
For the complete algorithmic procedure and implementation details, please refer to \shortautoref{appx:sample-mining}.


\paragraph{Semantic Reinforcement Stream.}
To refine regression accuracy without overfitting to outliers, this stream targets "hard" yet representative samples. We employ a \textit{Density-Aware Stratified Sampling} strategy on the error manifold. Focusing on samples where the error exceeds the population mean ($L_{mse}^{(i)} > \mu$), we partition the error distribution into $K$ bins and assign sampling quotas proportional to density:
\begin{equation}
...
\input{figures/RDFR}
\subsubsection{Temporal Refinement Reward System}
To align the model's policy with the standards of time-series forecasting, we employ Group Relative Policy Optimization (GRPO)~\citep{deepseekai2025deepseekr1} as our Reinforcement Forecasting Refinement framework. Therefore, we design a forecasting refinement reward function $R_{total}$ comprising three distinct components, each targeting a specific failure mode observed in STIT models.


\paragraph{Sequence Length Consistency Reward ($R_{len}$).}
...
\paragraph{Format Compliance Reward ($R_{fmt}$).}
To overcome the "sparse signal" problem where binary validity checks fail to distinguish between total hallucination and minor formatting errors, we employ \textit{reward shaping}. We decompose structural compliance into progressive sub-goals to provide dense gradient feedback:
...
\paragraph{Prediction Accuracy Reward ($R_{acc}$).}
